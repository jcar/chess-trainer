...
async function main() {
  ...
  const mp3 = `${e.key}.mp3`;
  const wav = `${e.key}.wav`;
  if (existsSync(join(audioDir, mp3)) || existsSync(join(audioDir, wav))) {
    reused++;
    continue;
  }
  ...
  if (haveFfmpeg) {
    ...
    if (r.status === 0) rmSync(join(audioDir, wav));
  }
  generated++;
  ...
  // Build the clip map from what's actually ON DISK for current manifest keys.
  // (Crucially NOT from this run's clipFiles — an early stop on a quota cap must
  // still record every clip already rendered, and must never drop a clip just
  // because the run didn't reach its key.)
  const manifestKeys = new Set(manifest.map((e) => e.key));
  const present = readdirSync(audioDir);
  const onDisk: Record<string, string> = {};
  for (const f of present) {
    const stem = f.replace(/\.(mp3|wav)$/, "");
    if (manifestKeys.has(stem)) {
      // Prefer mp3 over wav if both somehow exist.
      if (!onDisk[stem] || f.endsWith(".mp3")) onDisk[stem] = f;
    } else {
      // True orphan — its key is no longer in the manifest. Safe to remove.
  ...
  const body = Object.keys(onDisk)
    .sort()
    .map((k) => `  ${JSON.stringify(k)}: ${JSON.stringify(onDisk[k])},`)
    .join("\n");
  writeFileSync(
    clipManifestPath,
  ...
  console.log(
    `\nDone in ${mm}m${ss}s: ${generated} generated, ${reused} reused, ` +
      `${Object.keys(onDisk).length}/${manifest.length} total clips on disk.`,
  );
}
